...
async function updatePolitics() {
  try {
    const currentDate = new Date();
    await Politics.updateMany({}, { $set: { updatedAt: currentDate } });
  } catch (error) {
...
const getAllPolitics = async (req, res) => {
  const news = await Politics.find({}).sort({ createdAt: -1 });
  res.status(200).json(news);
};

const getOnePolitics = async (req, res) => {
  const { title } = req.params;
  const regex = new RegExp(title, "i");
  const news = await Politics.findOne({ slug: regex }).populate(
    "comments.postedBy",
    "email"
  );
...
const postonePolitics = async (req, res) => {
  let photo;
  let uploadedVideo;
  let result;
  const {
  ...
  try {
    ...
    if (video) {
      const news = await Politics.create({
        title,
        body,
        author,
        metaKey,
...
const deletePolitics = async (req, res) => {
  ...
  const lyricsid = await Politics.findByIdAndDelete({ _id: id });
  if (!lyricsid) {
    return res.status(404).json({ error: "No such news" });
  }
  const news = await Politics.find({}).sort({ createdAt: -1 });
...
const addComent = async (req, res, next) => {
  ...
  try {
    const news = await Politics.findByIdAndUpdate(
      req.params.id,
      {
        $push: {
          comments: {
