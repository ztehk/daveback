...
async function updateGist() {
  try {
    ...
    await Gist.updateMany({}, { $set: { updatedAt: currentDate } });
  } catch (error) {
    console.error("Error updating updatedAt:", error);
  }
}

const getAllGist = async (req, res) => {
  const news = await Gist.find({}).sort({ createdAt: -1 });
  res.status(200).json(news);
};

const getOneGist = async (req, res) => {
  const { title } = req.params;
  const regex = new RegExp(title, "i");
  const news = await Gist.findOne({ slug: regex }).populate(
    "comments.postedBy",
    "email"
  );
...
const postoneGist = async (req, res) => {
  let photo;
  let uploadedVideo;
  let result;
  const {
...
const deleteGist = async (req, res) => {
  const { id } = req.params;
  if (!mongoose.Types.ObjectId.isValid(id)) {
    return res.status(404).json({ error: "No such news!" });
  }
  const lyricsid = await Gist.findByIdAndDelete({ _id: id });
  if (!lyricsid) {
    return res.status(404).json({ error: "No such news" });
  }
  const news = await Gist.find({}).sort({ createdAt: -1 });
...
const addComent = async (req, res, next) => {
  ...
  try {
    const news = await Gist.findByIdAndUpdate(
      req.params.id,
      {
        $push: {
          comments: {
