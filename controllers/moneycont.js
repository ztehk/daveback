...
async function updateMoney() {
  try {
    ...
    await Money.updateMany({}, { $set: { updatedAt: currentDate } });
  } catch (error) {
...
const getAllMoney = async (req, res) => {
  const news = await Money.find({}).sort({ createdAt: -1 });
  res.status(200).json(news);
};

const getOneMoney = async (req, res) => {
  const { title } = req.params;
  const regex = new RegExp(title, "i");
  const news = await Money.findOne({ slug: regex }).populate(
    "comments.postedBy",
    "email"
  );
...
const postoneMoney = async (req, res) => {
  let photo;
  let uploadedVideo;
  let result;
  const {
...
const deleteMoney = async (req, res) => {
  ...
  const lyricsid = await Money.findByIdAndDelete({ _id: id });
  if (!lyricsid) {
    return res.status(404).json({ error: "No such news" });
  }
  const news = await Money.find({}).sort({ createdAt: -1 });
...
const addComent = async (req, res, next) => {
  ...
  try {
    const news = await Money.findByIdAndUpdate(
      req.params.id,
      {
        $push: {
          comments: {
