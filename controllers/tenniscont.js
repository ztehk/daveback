...
async function updateTennis() {
  try {
    ...
    await Tennis.updateMany({}, { $set: { updatedAt: currentDate } });
  } catch (error) {
    console.error("Error updating updatedAt:", error);
  }
}

const getAllTennis = async (req, res) => {
  const news = await Tennis.find({}).sort({ createdAt: -1 });
  res.status(200).json(news);
};

const getOneTennis = async (req, res) => {
  const { title } = req.params;
  const regex = new RegExp(title, "i");
  const news = await Tennis.findOne({ slug: regex }).populate(
    "comments.postedBy",
    "email"
  );
...
const postoneTennis = async (req, res) => {
  let photo;
  let uploadedVideo;
  let result;
  const {
  ...
  try {
    ...
    if (video) {
      const news = await Tennis.create({
        title,
        body,
        author,
        metaKey,
...
const deleteTennis = async (req, res) => {
  const { id } = req.params;
  if (!mongoose.Types.ObjectId.isValid(id)) {
    return res.status(404).json({ error: "No such news!" });
  }
  const lyricsid = await Tennis.findByIdAndDelete({ _id: id });
  if (!lyricsid) {
    return res.status(404).json({ error: "No such news" });
  }
  const news = await Tennis.find({}).sort({ createdAt: -1 });
...
const addComent = async (req, res, next) => {
  ...
  try {
    const news = await Tennis.findByIdAndUpdate(
      req.params.id,
      {
        $push: {
          comments: {
