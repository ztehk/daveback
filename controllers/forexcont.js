...
async function updateForex() {
  try {
    ...
    await Forex.updateMany({}, { $set: { updatedAt: currentDate } });
  } catch (error) {
...
const getAllForex = async (req, res) => {
  const news = await Forex.find({}).sort({ createdAt: -1 });
  res.status(200).json(news);
};

const getOneForex = async (req, res) => {
  const { title } = req.params;
  const regex = new RegExp(title, "i");
  const news = await Forex.findOne({ slug: regex }).populate(
    "comments.postedBy",
    "email"
  );
...
const postoneForex = async (req, res) => {
  let photo;
  let uploadedVideo;
  let result;
  const {
...
const deleteForex = async (req, res) => {
  ...
  const lyricsid = await Forex.findByIdAndDelete({ _id: id });
  if (!lyricsid) {
    return res.status(404).json({ error: "No such news" });
  }
  const news = await Forex.find({}).sort({ createdAt: -1 });
...
const addComent = async (req, res, next) => {
  ...
  try {
    const news = await Forex.findByIdAndUpdate(
      req.params.id,
      {
        $push: {
          comments: {
