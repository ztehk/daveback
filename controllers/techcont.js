...
async function updateTech() {
  try {
    ...
    await Tech.updateMany({}, { $set: { updatedAt: currentDate } });
  } catch (error) {
...
const getAllTech = async (req, res) => {
  const news = await Tech.find({}).sort({ createdAt: -1 });
...
const getOneTech = async (req, res) => {
  ...
  const news = await Tech.findOne({ slug: regex }).populate(
    "comments.postedBy",
    "email"
  );
...
const postOneTech = async (req, res) => {
  let photo;
  let uploadedVideo;
  let result;
  const {
...
const deleteTech = async (req, res) => {
  ...
  const lyricsid = await Tech.findByIdAndDelete({ _id: id });
  if (!lyricsid) {
    return res.status(404).json({ error: "No such news" });
  }
  const news = await Tech.find({}).sort({ createdAt: -1 });
...
const addComent = async (req, res, next) => {
  ...
  try {
    const news = await Tech.findByIdAndUpdate(
      req.params.id,
      {
        $push: {
          comments: {
