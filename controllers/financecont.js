...
async function updateFinance() {
  try {
    ...
    await Finance.updateMany({}, { $set: { updatedAt: currentDate } });
  } catch (error) {
...
const getAllFinance = async (req, res) => {
  const news = await Finance.find({}).sort({ createdAt: -1 });
...
const getOneFinance = async (req, res) => {
  ...
  const news = await Finance.findOne({ slug: regex }).populate(
    "comments.postedBy",
    "email"
  );
...
const postoneFinance = async (req, res) => {
  let photo;
  let uploadedVideo;
  let result;
  const {
  ...
  try {
    ...
    if (video) {
      const news = await Finance.create({
        title,
        body,
        author,
        metaKey,
...
const deleteFinance = async (req, res) => {
  ...
  const lyricsid = await Finance.findByIdAndDelete({ _id: id });
  if (!lyricsid) {
    return res.status(404).json({ error: "No such news" });
  }
  const news = await Finance.find({}).sort({ createdAt: -1 });
...
const addComent = async (req, res, next) => {
  ...
  try {
    const news = await Finance.findByIdAndUpdate(
      req.params.id,
      {
        $push: {
          comments: {
