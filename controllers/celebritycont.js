...
const Celeb = require("../models/celebritymodel");

async function updateCeleb() {
  try {
    const currentDate = new Date();
    await Celeb.updateMany({}, { $set: { updatedAt: currentDate } });
  } catch (error) {
    console.error("Error updating updatedAt:", error);
  }
}

const getAllCeleb = async (req, res) => {
  const news = await Celeb.find({}).sort({ createdAt: -1 });
  res.status(200).json(news);
};

const getOneCeleb = async (req, res) => {
  const { title } = req.params;
  const regex = new RegExp(title, "i");
  const news = await Celeb.findOne({ slug: regex }).populate(
    "comments.postedBy",
    "email"
  );
...
const patchoneceleb = async (req, res) => {
  const { id } = req.params;
  if (!mongoose.Types.ObjectId.isValid(id)) {
    return res.status(404).json({ error: "No such album!" });
  }
  const album = await Celeb.findByIdAndUpdate(
    { _id: id },
    {
      ...req.body,
    },
...
const postoneCeleb = async (req, res) => {
  let photo;
  let uploadedVideo;
  let result;
  const {
...
const deleteCeleb = async (req, res) => {
  ...
  const lyricsid = await Celeb.findByIdAndDelete({ _id: id });
  if (!lyricsid) {
    return res.status(404).json({ error: "No such news" });
  }
  const news = await Celeb.find({}).sort({ createdAt: -1 });
...
const addComent = async (req, res, next) => {
  ...
  try {
    const news = await Celeb.findByIdAndUpdate(
      req.params.id,
      {
        $push: {
          comments: {
