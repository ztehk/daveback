...
async function updateCourses() {
  try {
    ...
    await Courses.updateMany({}, { $set: { updatedAt: currentDate } });
  } catch (error) {
...
const getAllCourses = async (req, res) => {
  const news = await Courses.find({}).sort({ createdAt: -1 });
...
const getOneCourses = async (req, res) => {
  ...
  const news = await Courses.findOne({ slug: regex }).populate(
    "comments.postedBy",
    "email"
  );
...
const postoneCourses = async (req, res) => {
  let photo;
  let uploadedVideo;
  let result;
  const {
  ...
  try {
    ...
    if (video) {
      const news = await Courses.create({
        title,
        body,
        author,
        metaKey,
...
const deleteCourses = async (req, res) => {
  ...
  const lyricsid = await Courses.findByIdAndDelete({ _id: id });
  if (!lyricsid) {
    return res.status(404).json({ error: "No such news" });
  }
  const news = await Courses.find({}).sort({ createdAt: -1 });
...
const addComent = async (req, res, next) => {
  ...
  try {
    const news = await Courses.findByIdAndUpdate(
      req.params.id,
      {
        $push: {
          comments: {
